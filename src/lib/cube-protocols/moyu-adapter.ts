...
export class MoyuAdapter extends BaseAdapter {
  ...
  async connect(options: AdapterConnectOptions = {}): Promise<void> {
    ...
    if (!isMoyuCube) {
      ...
      throw new Error(
        `Connected device "${deviceName}" is not a MoYu cube. Please select a MoYu cube (WCU_*) or choose a different brand.`
      )
    }
    ...
    this.connection.events$.subscribe({
      ...
      error: (err) => {
        console.error('[moyu] Connection error:', err)
        this.emitDisconnect()
      },
      complete: () => {
        this.emitDisconnect()
